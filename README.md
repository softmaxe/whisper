...
Unused upstream pages, meeting and calendar services, cloud API/CLI guides, model download tools, and Windows/Linux packaging have been removed. Some provider, account, model, and database modules remain dependencies of the shared implementations. Their presence does not enable those features. The optional [ASR shim examples](examples/) remain useful for custom servers.
...
`npm run clean` removes this checkout's build output and dependency cache. It does not remove application data or settings.
...
The GitHub Actions release workflow requires a `vX.Y.Z` tag matching both package manifests. It tests and packages the ARM64 app, verifies its signature and architecture, publishes the ZIP and checksum, then updates `Casks/whisper.rb` in [softmaxe/homebrew-tap](https://github.com/softmaxe/homebrew-tap).
