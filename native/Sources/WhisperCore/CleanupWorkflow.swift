...
extension WhisperApplication {
    ...
    func cleanupContext() -> CleanupContext { CleanupContext(interfaceLanguage: state.settings.language) }
    ...
    func cleanDictation(rawText: String, requestID: UUID, context suppliedContext: CleanupContext? = nil) -> @MainActor () async -> Void {
        ...
        return { [weak self] in
            ...
            self?.completeDictation(rawText: rawText, text: text, requestID: requestID)
        }
    }
