...
extension WhisperApplication {
    ...
    private func resolveShortcutTrigger(_ input: ShortcutInput) -> ShortcutTrigger? {
        if !state.dictation.phase.isActive {
            guard input.isDown, let binding = state.settings.shortcuts.compactMap({ try? ShortcutBinding($0) })
                .first(where: { $0.matches(input, pressed: pressedKeys) }) else { return nil }
        ...
        if input.isDown, state.dictation.gesture.isProvisional,
           let longer = state.settings.shortcuts.compactMap({ try? ShortcutBinding($0) })
                .first(where: { $0 != binding && $0.matches(input, pressed: pressedKeys) }) {
