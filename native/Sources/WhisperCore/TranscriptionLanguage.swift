...
public struct TranscriptionPreferences: Codable, Equatable, Sendable {
    ...
    public func requestOptions(dictionaryPrompt: String? = nil) -> TranscriptionOptions {
        ...
        let prompt = [chinesePromptBias, hints].compactMap { $0 }.filter { !$0.isEmpty }.joined(separator: " ")
        return TranscriptionOptions(language: base, prompt: prompt.isEmpty ? nil : prompt)
    }

    /// Classify only the capped vocabulary actually sent, without diluting short dictionaries with the language bias.
    func dictionaryPrompt(from sentPrompt: String?) -> String? {
        guard let sentPrompt else { return nil }
        guard let bias = chinesePromptBias, sentPrompt.hasPrefix(bias) else { return sentPrompt }
        let vocabulary = sentPrompt.dropFirst(bias.count).trimmingCharacters(in: .whitespacesAndNewlines)
        return vocabulary.isEmpty ? nil : vocabulary
    }

    private var chinesePromptBias: String? {
