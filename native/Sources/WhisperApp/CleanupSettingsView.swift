...
struct CleanupSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                Toggle(language.text("Enable text cleanup", "启用文本整理"), isOn: $draft.enabled)
                    .accessibilityIdentifier("cleanup-enabled")
                field(language.text("Server URL", "服务器地址")) { TextField("http://localhost:8080", text: $draft.serverURL).accessibilityIdentifier("cleanup-server-url") }
                field(language.text("Model", "模型")) { TextField("default", text: $draft.model).accessibilityIdentifier("cleanup-model") }
                field(language.text("API Key (Optional)", "API Key（可选）")) {
                    SecureField(application.state.cleanupCredentialConfigured ? language.text("Saved in Keychain", "已保存到钥匙串") : language.text("Enter API key", "输入 API Key"), text: $apiKey)
                ...
                if application.state.cleanupCredentialConfigured {
                    Toggle(language.text("Remove saved API key", "移除已保存的 API Key"), isOn: $removeKey)
                }
                Toggle(language.text("Disable thinking", "关闭思考"), isOn: $draft.disableThinking)
                HStack {
                    Text(language.text("Temperature", "温度"))
                    TextField("0", value: $draft.temperature, format: .number).frame(width: 90)
                    Text(language.text("Token limit", "Token 上限"))
                    TextField("4096", value: $draft.maxTokens, format: .number).frame(width: 110)
                }
            ...
            if promptTab == 0 {
                ...
                TextEditor(text: $prompt).frame(minHeight: 230).accessibilityIdentifier("cleanup-prompt-editor")
                HStack {
                    Button(language.text("Reset to default", "恢复默认")) {
                        application.send(.resetCleanupPrompt)
                        if application.state.settingsSaved {
                            draft.customPrompt = nil
                            prompt = CleanupPrompts.defaultText(in: language)
                        }
                    }
                    Spacer()
        ...
        .onChange(of: language) {
            if draft.customPrompt == nil { prompt = CleanupPrompts.defaultText(in: language) }
        }
    }
    ...
    private func restore() {
        draft = application.state.settings.cleanup
        prompt = draft.customPrompt ?? CleanupPrompts.defaultText(in: language)
        apiKey = ""
        removeKey = false
    }
    private func save() {
        application.send(.saveCleanup(draft, credential: removeKey ? .remove : apiKey.isEmpty ? .unchanged : .replace(apiKey)))
        if application.state.settingsSaved { apiKey = ""; removeKey = false }
    }
    private func savePrompt() {
        var settings = application.state.settings.cleanup
        settings.customPrompt = prompt == CleanupPrompts.defaultText(in: language) ? nil : prompt
        application.send(.saveCleanupPrompt(settings.customPrompt))
        if application.state.settingsSaved { draft.customPrompt = settings.customPrompt }
    }
}
