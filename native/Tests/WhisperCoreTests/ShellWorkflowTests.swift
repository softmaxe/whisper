...
struct ShellWorkflowTests {
    ...
    @Test func terminationJoinsReplacedStorageReadsBehindPendingAudioPersistence() async throws {
        let fixture = try ShellFixture(); defer { fixture.remove() }
        fixture.app.send(.setHistoryRetention(.init(saveDiscarded: true)))
        fixture.app.send(.startDictation)
        let capture = try #require(fixture.microphones.sessions.last)
        capture.open(); capture.deliver([Float](repeating: 0, count: 57_600))
        await settle { fixture.app.state.dictation.phase == .recording }
        ...
        fixture.app.send(.refreshPrivacy)
        fixture.app.send(.refreshPrivacy)
        #expect(fixture.app.state.privacy.isLoadingStorage)
        var finished = false
        let termination = Task { await fixture.app.prepareForTermination(); finished = true }
        await settle { fixture.app.state.isTerminating }
        #expect(!finished && capture.physicallyOpen)
        #expect(!fixture.app.state.privacy.isLoadingStorage)
        fixture.app.send(.refreshPrivacy)
        capture.finishRelease()
        await termination.value
        #expect(fixture.app.state.privacy.audioUsage == nil)
        #expect(!fixture.app.state.privacy.storageFailed)
        #expect(fixture.privacy.reads == 2)
        #expect(fixture.privacy.requests.isEmpty)
        let saved = try #require(try await HistoryStore(profile: fixture.profile.profile).entry(requestID))
