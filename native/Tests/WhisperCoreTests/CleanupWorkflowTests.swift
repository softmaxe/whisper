...
@Suite(.serialized) @MainActor struct CleanupWorkflowTests {
    ...
    @Test func teardownDuringChineseConversionReleasesApplication() async throws {
        let fixture = try CleanupFixture()
        defer { fixture.remove() }
        fixture.app.send(.setTranscriptionLanguage("zh-TW"))
        await fixture.transcribe(raw: "Conversion teardown fixture")
        await settle { await fixture.http.requests.count == 1 }
        await fixture.http.reply(content: String(repeating: "这是简体中文软件。", count: 100_000))
