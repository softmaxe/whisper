...
struct DiagnosticsWorkflowTests {
    ...
    @Test func terminationClosesDiagnosticsAfterRetainedAudioAndInsightsAreDurable() async throws {
        ...
        await f.app.prepareForTermination()
        let saved = try #require(try await HistoryStore(profile: f.profile.profile).entry(id))
        #expect(saved.hasAudio && saved.rawText == "private-speech-fixture")
        let rows = try String(contentsOf: f.file, encoding: .utf8).split(separator: "\n")
        let row = try #require(rows.last)
        let record = try #require(JSONSerialization.jsonObject(with: Data(row.utf8)) as? [String: Any])
        #expect(record["outcome"] as? String == "completed")
        let stages = try #require(record["stages"] as? [String: Double])
        let released = try #require(stages["captureReleased"])
        let finalized = try #require(stages["recordingFinalized"])
        let dispatched = try #require(stages["asrRequestDispatched"])
        #expect(released <= finalized && finalized <= dispatched)
        let reopened = f.profile.open()
        reopened.send(.loadInsights)
        await settle { !reopened.state.insights.isLoading }
        #expect(reopened.state.insights.summary?.totalDictations == 1)
        // A new writer can acquire the same file only after termination releases its flock.
        reopened.send(.setDiagnosticsOutput(f.file))
        #expect(await reopened.flushDiagnostics())
