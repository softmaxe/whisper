...
test("atomic registerSlot failure keeps the slot's previous working hotkeys", async () => {
  ...
  const first = await mgr.registerSlot("dictation", "F7", noop, { atomic: true });
  assert.equal(first.success, true);
  assert.deepEqual(mgr.getSlotHotkeys("dictation"), ["F7"]);
  assert.equal(registered.has("F7"), true);

  const second = await mgr.registerSlot("dictation", "F7,Control+BAD", noop, { atomic: true });
...
test("non-atomic registerSlot keeps the working subset and reports failures", async () => {
  ...
  const result = await mgr.registerSlot("dictation", "F7,Control+BAD", noop);
...
test("re-registering a slot replaces its previous accelerators", async () => {
  ...
  await mgr.registerSlot("dictation", "F7", noop, { atomic: true });
  const result = await mgr.registerSlot("dictation", "F6,F5", noop, { atomic: true });
  assert.equal(result.success, true);
  assert.deepEqual(mgr.getSlotHotkeys("dictation"), ["F6", "F5"]);
