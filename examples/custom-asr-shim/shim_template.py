#!/usr/bin/env python3
"""Local adapter for ASR backends that do not implement Whisper's
OpenAI-compatible /audio/transcriptions request format.

Whisper's request and response format:
  ...
  - Do not require Authorization from Whisper. Hold vendor keys in env vars.
  - It expects HTTP 200 with JSON {"text": "..."}. Empty text shows
    "No audio detected"; non-200 is surfaced as an API error.

Implement `transcribe()` for your backend. See README.md for setup and limits.
"""
...
def transcribe(audio_path: str, model: str, language: str | None, prompt: str | None) -> str:
    """Call your vendor's ASR backend with the WAV at
    `audio_path` and return the transcript as a plain string.

    `model`/`language`/`prompt` come from Whisper and may be
