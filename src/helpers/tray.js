...
class TrayManager {
  ...
  async createTray() {
    if (!this.visible || this.tray) return;
    if (this.createTrayPromise) {
      await this.createTrayPromise;
      if (this.visible && !this.tray) return this.createTray();
      return;
    }
    this.createTrayPromise = this._createTray().finally(() => {
      this.createTrayPromise = null;
    });
  ...
  setupTrayEventHandlers() {
    ...
    tray.on("destroyed", () => {
      ...
      if (this.tray === tray) this.tray = null;
    });
  }
}
