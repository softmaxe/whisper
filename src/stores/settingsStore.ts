...
export const useSettingsStore = create<SettingsState>()((set, get) => ({
  ...
  cleanupMode: "self-hosted" as InferenceMode,
  ...
  setShowMenuBarIcon: (visible: boolean) => {
    ...
    if (isBrowser) localStorage.setItem("showMenuBarIcon", String(visible));
...
export async function initializeSettings(): Promise<void> {
  ...
  if (window.electronAPI) {
    try {
      const [customTx, customRx] = await Promise.all([
    ...
    try {
      ...
      if (typeof visible === "boolean") {
        localStorage.setItem("showMenuBarIcon", String(visible));
